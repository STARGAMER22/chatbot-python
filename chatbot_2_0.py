...
class Chatbot2:
    ...
    def repondre(self, message: str) -> str:
        ...
        if any(mot in normalise for mot in ("bonjour", "salut", "hello", "bonsoir")):
            ...
            return f"{prefixe}  Salut !Comment se passe ta journée ?"
            return f"{prefixe}  En quoi puis-je t'être utile ?"

        # Questions factuelles simples
        if "qui es-tu" in normalise:
            self.dernier_sujet = "identite"
            return "Je suis Chatbot 2.0, un assistant conversationnel Python créé par STARGAMER22. Et toi, qui est tu ?"

        if "que peux-tu faire" in normalise:
        ...
        if any(expr in normalise for expr in ("je vais bien", "ça va bien", "ca va bien", "je suis content", "heureux", "satisfait")):
            self.dernier_sujet = "humeur_positive"
            return "J'adore ton humeur positive !!! 😄😄😄 C'est agréable..."
        ...
        if any(expr in normalise for expr in ("je vais mal", "ça va mal", "ca va mal", "triste", "stress", "fatigu")):
            ...
            return "Y a-t-il une chose qui te contrarie et dont tu voudrais me parler ?"
        ...
        if normalise in {"oui", "non", "peut-être", "bof"}:
            ...
            if self.dernier_sujet == "humeur_positive":
                return "J'adore ton énergie ! Continue comme ça ! 😄😄😄"
